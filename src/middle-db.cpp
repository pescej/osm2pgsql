                                     ...
                                     options_t const &options)
{
    ...
    tmpl->set("way_node_index_id_shift", std::to_string(options.way_node_index_id_shift));
}
...
void middle_db_t::after_nodes()
{
    ...
    task.set(
        m_thread_pool->submit(std::bind(exec_commands, m_conninfo, commands)));
...
middle_db_t::middle_db_t(options_t const *options, thread_pool_t *thread_pool)
: middle_t(thread_pool), m_conninfo(options->database_options.conninfo()),
  m_db_connection(m_conninfo),
  ...
  m_mode(options->append ? mode::update : mode::import)
{
    ...
    if (options->cache > 0) {
        ...
        m_max_cache = options->cache * 1024 * 1024;
    }
    ...
    if (m_store_options.drop_tables) {
        ...
        for (auto const type :
            ...
            if (!commands.front().empty()) {
                ...
                table.task_fw_dep_index().set(m_thread_pool->submit(
                    std::bind(exec_commands, m_conninfo, commands)));
            }
        }
        for (auto &table : m_tables) {
            auto const duration = table.task_fw_dep_index().wait().count();
            if (duration > 0) {
                log_info("Done postprocessing on table '{}' in {}",
                         table.name(),
                         util::human_readable_duration(duration / 1000));
            }
        }
    }
}
